...
public class FloatExtensionsTests : ExtensionTestSuite<float> {
    ...
    protected override void PeekSpanRaw(ReadContext context, int count, ref Span<float> destination) => context.PeekFloatSpanRaw(count, ref destination);
    protected override void ReadSpanRaw(ReadContext context, int count, ref Span<float> destination) => context.ReadFloatSpanRaw(count, ref destination);
    ...
    protected override void PeekSpanWithoutLength(ReadContext context, int count, ref Span<float> destination) => context.PeekFloats(count, ref destination);
    protected override void ReadSpanWithoutLength(ReadContext context, int count, ref Span<float> destination) => context.ReadFloats(count, ref destination);
    ...
    protected override void PeekSpanWithoutLengthAlias(ReadContext context, int count, ref Span<float> destination) => context.Peek(count, ref destination);
    protected override void ReadSpanWithoutLengthAlias(ReadContext context, int count, ref Span<float> destination) => context.Read(count, ref destination);
    protected override void TryPeekSpanWithoutLength(ReadContext context, int count, ref Span<float> destination) { Assert.True(context.TryPeek(count, ref destination)); }
    protected override void TryReadSpanWithoutLength(ReadContext context, int count, ref Span<float> destination) { Assert.True(context.TryRead(count, ref destination)); }
    protected override void TryPeekSpanWithoutLengthAlias(ReadContext context, int count, ref Span<float> destination) { Assert.True(context.TryPeek(count, ref destination)); }
    protected override void TryReadSpanWithoutLengthAlias(ReadContext context, int count, ref Span<float> destination) { Assert.True(context.TryRead(count, ref destination)); }
    protected override void WriteSpan(WriteContext context, Span<float> values) => context.WriteFloats(values);
    protected override void PeekSpanWithLength(ReadContext context, ref Span<float> destination) => context.PeekFloats(ref destination);
    protected override void ReadSpanWithLength(ReadContext context, ref Span<float> destination) => context.ReadFloats(ref destination);
    protected override void WriteSpanAlias(WriteContext context, Span<float> values) => context.Write(values);
    protected override void PeekSpanWithLengthAlias(ReadContext context, ref Span<float> destination) => context.Peek(ref destination);
    protected override void ReadSpanWithLengthAlias(ReadContext context, ref Span<float> destination) => context.Read(ref destination);
    protected override void TryPeekSpanWithLength(ReadContext context, ref Span<float> destination) { Assert.True(context.TryPeek(ref destination)); }
    protected override void TryReadSpanWithLength(ReadContext context, ref Span<float> destination) { Assert.True(context.TryRead(ref destination)); }
    protected override void TryPeekSpanWithLengthAlias(ReadContext context, ref Span<float> destination) { Assert.True(context.TryPeek(ref destination)); }
    protected override void TryReadSpanWithLengthAlias(ReadContext context, ref Span<float> destination) { Assert.True(context.TryRead(ref destination)); }
