...
internal readonly struct Helpers<T>(ImmutableArray<T> array) : IEquatable<Helpers<T>>, IEnumerable<T> where T : IEquatable<T> {
    private readonly ImmutableArray<T> _array = array;

    public bool Equals(Helpers<T> other) { return _array.SequenceEqual(other._array); }
    public override bool Equals(object? obj) { return obj is Helpers<T> other && Equals(other); }
    ...
    public static implicit operator Helpers<T>(ImmutableArray<T> array) => new(array);
    public static implicit operator ImmutableArray<T>(Helpers<T> array) => array._array;
}
